...
template <class T>
class TreeReverseIterator : public ft::iterator< std::bidirectional_iterator_tag, T>
{
public:    
    // nodePtr is the current location in the tree. we can move
    // freely about the tree using left, right, and parent.
    ...
    // used to construct an iterator return value from a node pointer
    TreeReverseIterator (Node *p, Node *r) : nodePtr(p), root(r) {};
	
	TreeReverseIterator() {};
	TreeReverseIterator(const TreeReverseIterator& other) : nodePtr(other.nodePtr), root(other.root) {};
	...
	friend bool operator==(TreeReverseIterator lhs, TreeReverseIterator rhs) {return (lhs.nodePtr == rhs.nodePtr);}
    ...
    TreeReverseIterator operator--(int) {
		TreeReverseIterator clone(*this);
		--(*this);
		return (clone);
	}
